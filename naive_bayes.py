...
class NaiveBayes():
    ...
    def fit(self, train_hams: list, train_spams: list):
        ...
        def get_counts(filenames: list):
            ...
            # to get the number of emails, we can take the length of the filenames list
            return len(filenames)

        pass  # TODO: Your code here (10-20 lines)

        # set num_train_hams to the number of ham emails
        self.num_train_hams = get_counts(train_hams)

        # set num_train_spams to the number of spam emails
        self.num_train_spams = get_counts(train_spams)

        # iterate through the ham emails
        for i in range(self.num_train_hams):

            # get the word_set of the ham email
            for word in self.word_set(train_hams[i]):

                # set the word_counts_ham value of the word key to 0 if we don't have a value yet
                # otherwise increment the value
                self.word_counts_ham[word] =self.word_counts_ham.get(word,0)+1

        # iterate through the spam emails
        for i in range(self.num_train_spams):

            # get the word set for each spam email
            for word in self.word_set(train_spams[i]):

                # set the word_counts_spam value of the word key to 0 if there is no value yet
    ...
    def predict(self, filename: str):
        ...
        # set pHam to the number of training ham emails divided by the total number of training emails
        pHam = self.num_train_hams / (self.num_train_hams + self.num_train_spams)

        # set pSpam to the number of training spam emails divided by the total number of training emails
        pSpam = self.num_train_spams / (self.num_train_hams + self.num_train_spams)

        # use the log trick but not Laplace smoothing on pHam and pSpam to prevent underflow
        # get the logs of pHam and pSpam
        pHamLogTotal = np.log(pHam)
        pSpamLogTotal = np.log(pSpam)

        

        # iterate through the word_set for the file
        for word in self.word_set(filename):

            # increase the log sums by the logs of the conditional probabilities for each word in the word set
            # use the log trick to prevent underflow and Laplace smoothing so that we don't have probabilities of 0
            # set the value to 0 if the word key does not exist
            pCondHam=(self.word_counts_ham.get(word, 0) + 1) / (self.num_train_hams + 2)
            pHamLogTotal += np.log(pCondHam)
            pCondSpam=(self.word_counts_spam.get(word, 0) + 1) / (self.num_train_spams + 2)
            pSpamLogTotal += np.log(pCondSpam)

        # if the log sum of pSpam is greater than the log sum of pHam
        if pSpamLogTotal > pHamLogTotal:

            # predict spam
            return self.SPAM_LABEL

        # else probability of ham is greater, or we have a tie, so predict ham
